...
static void ANATOP_PllSetPower(anatop_ai_itf_t itf, bool enable)
{
    ANATOP_AI_Write(itf, enable ? 0x04U : 0x08U, 0x4000UL | (enable ? 0x2000UL : 0UL));
}

static void ANATOP_PllBypass(anatop_ai_itf_t itf, bool bypass)
{
    ANATOP_AI_Write(itf, bypass ? 0x04U : 0x08U, 0x10000UL);
}

static void ANATOP_PllEnablePllReg(anatop_ai_itf_t itf, bool enable)
{
    ANATOP_AI_Write(itf, enable ? 0x04U : 0x08U, 0x400000UL);
}

static void ANATOP_PllHoldRingOff(anatop_ai_itf_t itf, bool off)
{
    ANATOP_AI_Write(itf, off ? 0x04U : 0x08U, 0x2000UL);
}
...
{
    ANATOP_AI_Write(itf, enable ? 0x04U : 0x08U, 0x8000UL);
}
    ...
    anatop_ai_itf_t itf, uint8_t div, uint32_t numer, uint8_t post_div, uint32_t denom, const clock_pll_ss_config_t *ss)
{
    ...
    if (itf != kAI_Itf_1g)
    {
        ANATOP_AI_WriteWithMaskShift(itf, 0x00U, post_div, 0xE000000UL, 25UL);
    }
...
static uint32_t CLOCK_GetAvPllFreq(clock_pll_t pll)
{
    ...
    div      = ANATOP_AI_Read(pll == kCLOCK_PllAudio ? kAI_Itf_Audio : kAI_Itf_Video, 0U);
    post_div = (div & (0xE000000UL)) >> 25UL;
    div &= 0x7fUL;

    denom = (double)ANATOP_AI_Read(pll == kCLOCK_PllAudio ? kAI_Itf_Audio : kAI_Itf_Video, 0x30U);
    numer = (double)ANATOP_AI_Read(pll == kCLOCK_PllAudio ? kAI_Itf_Audio : kAI_Itf_Video, 0x20U);

    freq = (uint32_t)((uint64_t)XTAL_FREQ * ((uint64_t)div + (uint64_t)(double)(numer / denom)) / (1ULL << post_div));
    return freq;
}
